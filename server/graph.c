...
FILE* tryOpenFile(const char *filename){
    ...
    if (file == NULL) {
        ...
        scheme[0].type = BOOL;
        strncpy(scheme[0].name, "isNew\0", MAX_NAME_LENGTH);
        scheme[1].type = FLOAT;
        strncpy(scheme[1].name, "Cost\0", MAX_NAME_LENGTH);
        scheme[2].type = INT32;
        strncpy(scheme[2].name, "IDModel\0", MAX_NAME_LENGTH);
        scheme[3].type = CHAR_PTR;
        strncpy(scheme[3].name, "Name\0", MAX_NAME_LENGTH);
        scheme[4].type = INT32;
        strncpy(scheme[4].name, "NumberInBase\0", MAX_NAME_LENGTH);
        struct GraphDB db = {0};
        ...
        switch (column->type) {
            case INT32:
                if (sscanf(token, "%d", &column->int32Value) != 1) {
                    ...
                    sprintf(tmp, "Error while adding!\nArgument %d has wrong type (INT32 expected).\n", columnIndex);
            ...
            case FLOAT:
                if (sscanf(token, "%f", &column->floatValue) != 1) {
                    free(inputStringCopy);
                    sprintf(tmp, "Error while adding!\nArgument %d has wrong type (FLOAT expected).\n", columnIndex);
                    *response = concat(*response, tmp);
                    return false;
                }
                break;
            ...
            case BOOL:
                if (strcmp(token, "true") == 0) {
                    ...
                    free(inputStringCopy);
                    sprintf(tmp, "Error while adding!\nArgument %d has wrong type (BOOL expected).\n", columnIndex);
                    *response = concat(*response, tmp);
                    return false;
                }
                break;
